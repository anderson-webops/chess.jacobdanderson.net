...
class RuntimeArtifactTests(unittest.TestCase):
    def setUp(self):
        ...
        required = self.contract["required"] + [p.replace("*", "fixture") for p in self.contract.get("requiredPatterns", [])]
        ...
        (self.root / ".chess-release-prepared.json").write_text(marker)
    ...
    def test_missing_worker_even_if_inventory_omits_it(self):
        for pattern in self.contract["requiredPatterns"]:
            for path in self.root.glob(pattern):
                path.unlink()
        with self.assertRaisesRegex(ValueError, "required runtime pattern missing"):
            artifact.validate(self.root, self.manifest())
    ...
    def test_malformed_identity_cannot_be_rehashed(self):
        for marker in [{}, {"release": "v1.0.0", "commitSha": "a" * 40},
            ...
            with self.subTest(marker=marker):
                for name in ["front-end/.output/public/release.json", ".chess-release-prepared.json"]:
