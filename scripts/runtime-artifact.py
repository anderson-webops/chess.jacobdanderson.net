...
def permitted(name):
    ...
    if not parts or PurePosixPath(name).as_posix() != name or name.startswith("/"):
        return False
    top = parts[0]
    allowed = top in ("package.json", "package-lock.json", ".chess-release-prepared.json", MANIFEST) and len(parts) == 1
...
def validate(root, manifest):
    ...
    for pattern in contract.get("requiredPatterns", []):
        if not any(fnmatch.fnmatchcase(name, pattern) for name in actual):
            raise ValueError(f"required runtime pattern missing: {pattern}")
    ...
    marker = json.loads((root / ".chess-release-prepared.json").read_text())
