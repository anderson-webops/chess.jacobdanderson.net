...
timeout -k 5 90 bwrap --unshare-all --die-with-parent --new-session \
  ...
  --ro-bind "$script_dir/artifact-acceptance/nginx.mjs" /harness/nginx.mjs \
  --ro-bind "$script_dir/../deploy/nginx/chess.jacobdanderson.net.server.conf" /harness/nginx-site.conf \
  --ro-bind /etc/nginx/mime.types /harness/mime.types \
